...
function displayForecast(responseJson, itDesc, itStartDate, itEndDate) {
  for (i=0; i < responseJson?.length; i++) {
    ...
    if (responseJson[i].observation_time.value >= itStartDate && responseJson[i].observation_time.value <= itEndDate){
      ...
      $('.js-results').append('<div class="forecast-pair"><div class="forecast-text"><h3>' + itDesc + ' on ' + responseJson[i].observation_time.value.slice(5, 10) + '</h3><ul><li>Overview: <strong>' + prettyWeatherCode + '</strong></li><li>' + responseJson[i].precipitation_probability.value +  responseJson[i].precipitation_probability.units + ' chance of precipitation</li><li>"Feels Like" temperature:</li><ul><li>min: ' + responseJson[i].feels_like[0].min.value + ' &#8457;</li><li>max: ' + responseJson[i].feels_like[1].max.value + ' &#8457;</li></ul></ul></div><div class="forecast-picture"><img src="assets/sample.jpg"></div></div>');
    };
  }
